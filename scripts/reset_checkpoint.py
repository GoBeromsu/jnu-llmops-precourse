...
BLOCK-ID 별 동작:
    D1-FMT      app.py 의 Commit 되지 않은 변경만 버린다 (git restore app.py). 여러분의 Commit 은 남는다.
...
def restore_day1():
    result = subprocess.run(
        ["git", "restore", "--", "app.py"], cwd=ROOT, capture_output=True, text=True
    )
...
def restore_from_checkpoint(block_id):
    ...
    for src_name, dst_name in pairs:
        ...
        dst = ROOT / dst_name
        if not src.is_file():
            return fail(f"{block_id}: 정본 File 이 없습니다 — {src}")
